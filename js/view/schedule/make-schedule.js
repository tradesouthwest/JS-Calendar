...
FormView.prototype = {
    ...
    compareTime: function() {
        ...
        var a = time.split(':');
        var ts = ((a[0] * 60) * 60) + (a[1] * 60); // Convert the current time to seconds
        if (ts >= 82800) { // Convert the desired time to seconds and compare the current time to the value converted to seconds
...
Submission.prototype = {
    ...
    isTarget: function(event) {
        ...
        || (event.target.parentNode.className === "fc-more-cell") 
        || (event.target.parentNode.className === "fc-hide-cell")) {
            return true;
        } else return false;
    },
    timeAlert: function() {
    ...
    compareDate: function(event) {
        ...
        if ((timeStart - timeEnd) >= 0) {
            alert('date time event may be off')
    ...
    saveScheduleInfo: function(x) {
        ...
        if (scheduleInfo === undefined) return 0; // if the recurring schedule period is wrong
    ...
    getScheduleInfo: function() {
        ...
        if (scheduleInfo["title"].length === 0) {
            scheduleInfo["title"] = "Untitled schedule";
        }
        var timeArray = this.getTime();
    ...
    /** Period check in case of recurring schedule */
    checkRepeatEvent: function(repeat, start, end) {
        ...
        if (!result) alert("The date value is invalid.");
        return result;
    }
};
